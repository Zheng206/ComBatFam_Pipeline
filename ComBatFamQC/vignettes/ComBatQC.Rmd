...
-   `gam`: Generalized additive model, which models the relationship between the dependent variable and each independent variable as a smooth, non-linear function, typically using splines.
-   `lmer`: Linear mixed-effects model, which extends the linear regression model to account for both fixed effects and random effects. It us commonly used in longitudinal datasets.
...
-   Out of Sample Harmonization
    ...
    -   harmonize new data toward existing reference data (works only for all default combat harmonization methods)
...
harm_predict = combat_harm(type = "gam", features = features, batch = batch, covariates = covariates, interaction = interaction, smooth_int_type = "linear", smooth = "AGE", df = adni %>% head(1000), predict = TRUE, object = saved_model)
```
...
features = colnames(reference_df)[c(8:69)]
covariates = c("timedays", "AGE", "SEX", "DIAGNOSIS")
interaction = c("timedays,DIAGNOSIS")
batch = "site"
